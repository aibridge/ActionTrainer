...
class WebcamLoader:
    def __init__(self, webcam, batchSize=1, queueSize=256):
        ...
        assert self.stream.isOpened(), 'Cannot capture source, please set another webcam number(default is zero, you can set a number with --webcam command)'
        self.stopped = False
...
class WebcamDetectionLoader:
    def __init__(self, webcam = 1, batchSize=1, queueSize=256):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
        self.det_model = Darknet("yolo/cfg/yolov3-spp.cfg")
        self.det_model.load_weights('models/yolo/yolov3-spp.weights')
...
class DataWriter:
    def __init__(self,C_T, T_s, T_p, pose_id, save_video=False,
                savepath='examples/res/1.avi', fourcc=cv2.VideoWriter_fourcc(*'XVID'), fps= 25, frameSize=(256,256), total=0,
                queueSize=1024):
        if save_video:
            # initialize the file video stream along with the boolean
            # used to indicate if the thread should be stopped or not
        ...
        self.T_pose = T_p
        self.iter =0
        self.T_series = np.zeros(shape=(17,30))
        self.U_series = np.zeros(shape=(17,30))
        self.total_len = total
        self.Cap_T = C_T
        self.pose_id_ =  pose_id
        # initialize the queue used to store frames read from
        # the video file
